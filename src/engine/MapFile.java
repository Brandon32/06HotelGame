...
/**
 * File utilities for reading and writing the game map
 * 
...
{
    /**
     * Read the game map into a byte array. This method works on a local game map file and a game map file in a JAR
     * 
     * @param obj
     * @param fileName
     * @return byte [ ][ ]
     */
    public static byte [ ][ ] readMap( Object obj, String fileName )
    {
        int rows;
        int columns;
        String line;
        ...
        try
        {
            ...
            for ( int row = 0; row < map.length; row++ )
            {
                ...
                {
                    map[ row ][ index ] = Byte.parseByte( pieces[ index ].trim() );
                }
            }
    ...
    /**
     * Write the game map to a file. This method will not write to a file inside a jar
     * 
     * @param obj
     * @param fileName
     * @param map
     */
    public static void writeMap( Object obj, String fileName, byte map[][] )
    {
        try
        {
            ...
            try
            {
                ...
                for ( int row = 0; row < map.length; row++ )
                {
                    ...
                    {
                        out.write( Byte.toString( map[ row ][ index ] ) );
