...
public class GameDisplay {
	...
	private static ArrayList<GameEventKeyboard> keyListeners = new ArrayList<GameEventKeyboard>();
	private static MouseEvent lastMouseEvent;
	private static ArrayList<GameEventMouse> mouseListeners = new ArrayList<GameEventMouse>();
	 ...
	 * @param kel
	 */
	public static synchronized void addKeyboardListener(GameEventKeyboard kel) {
	 ...
	 * @param mel
	 */
	public static synchronized void addMouseListener(GameEventMouse mel) {
	...
	public static void title(String value) {
		frameInstance.setTitle(value);
	}

	public static void icon(BufferedImage image) {
		frameInstance.setIconImage(image);
	}
	...
	public static void create(int width, int height) {
		if (frameInstance == null) {
			...
			try {
				robot = new Robot();
				// robot.mouseMove( frameInstance.getX() +
				// frameInstance.getWidth() / 2, frameInstance.getY() +
				// frameInstance.getHeight() / 2 );
			} catch (AWTException e) {
				throw new RuntimeException(e.getMessage());
			}

			/**
			 * Add a mouse motion listener so we can get mouse motion
			 */
			frameInstance.addMouseMotionListener(new MouseMotionAdapter() {
				...
				public void mouseDragged(MouseEvent me) {
					 ...
					 */
					if (!frameInstance.getBounds().contains(me.getXOnScreen(),
							me.getYOnScreen())) {
						processMouseEvent(me);
					} else {
						for (GameEventMouse listener : mouseListeners) {
			...
			frameInstance.addKeyListener(new KeyAdapter() {
				...
				public void keyPressed(KeyEvent ke) {
					for (GameEventKeyboard listener : keyListeners) {
	...
	public static Dimension getBounds() {
		if (frameInstance != null) {
			return new Dimension(frameInstance.getWidth(),
	...
	private static void processMouseEvent(MouseEvent me) {
		...
		if (captureCursor && !me.isShiftDown()) {
			robot.mouseMove(frameInstance.getX() + lastMouseEvent.getX(),
					frameInstance.getY() + lastMouseEvent.getY());
		}
	}
